...
Region <- s_reg
Regiontab1 <- subset(table1, table1$NEAFCregion == Region_ID)

# get VME polygons in the region
scen11_R <- st_make_valid(st_intersection(s_scen11,Region))
scen12_R <- st_make_valid(st_intersection(s_scen12,Region))
scen21_R <- st_make_valid(st_intersection(s_scen21,Region))
scen22_R <- st_make_valid(st_intersection(s_scen22,Region))
scen23_R <- st_make_valid(st_intersection(s_scen23,Region))

# get fishable domain in the region
Fishdom <- st_intersection(eval(parse(text = paste("s_",FishD,"Footp",sep=""))),Region)
...
Mobile_fish <- s_New_mobile ; Mobile_fish <- st_make_valid(Mobile_fish)
Static_fish <- s_New_static ; Static_fish <- st_make_valid(Static_fish)
...
if (FishD == "NEAFC"){
  ...
  cl_scen12 <- st_intersection(scen12_R,s_clos_neafc); cl_scen12 <- st_make_valid(cl_scen12)
  tab2[5,3] <- round(sum(st_area(cl_scen12))/sum(st_area(scen12_R)) * 100,digits = 1)
  
  cl_scen21 <- st_intersection(scen21_R,s_clos_neafc); cl_scen21 <- st_make_valid(cl_scen21)
  tab2[5,4] <- round(sum(st_area(cl_scen21))/sum(st_area(scen21_R)) * 100,digits = 1)
  
  cl_scen22 <- st_intersection(scen22_R,s_clos_neafc); cl_scen22 <- st_make_valid(cl_scen22)
  tab2[5,5] <- round(sum(st_area(cl_scen22))/sum(st_area(scen22_R)) * 100,digits = 1)
  
  cl_scen23 <- st_intersection(scen23_R,s_clos_neafc); cl_scen23 <- st_make_valid(cl_scen23)
...
if (nrow(sce11@data !=0)){
sce11@data$area_sqkm <- area(sce11)/10^6
} else {
sce11@data$area_sqkm <- 0
}
...
if (nrow(sce12@data !=0)){
  sce12@data$area_sqkm <- area(sce12)/10^6
} else {
  sce12@data$area_sqkm <- 0
}
