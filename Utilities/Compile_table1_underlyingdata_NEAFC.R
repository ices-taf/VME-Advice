...
depth <- depth_Gebco
depth$mean_depth <- depth$GebcoAtlan *-1

IREG <- subset(depth,!(depth$mean_depth > 200))
IREG <- subset(IREG, !(IREG$mean_depth  < 50)) 
...
# get area 200-400  
IREG <- subset(depth,!(depth$mean_depth > 400))
IREG <- subset(IREG, !(IREG$mean_depth  < 200)) 
IREG$within <- 1  # if TRUE
depth <- cbind(depth,IREG[match(depth$csquare,IREG$csquare),c("within")])
...
bargrid <- cbind(bargrid,depth[match(bargrid@data$csquares,depth$csquares),c("cat","mean_depth")])
