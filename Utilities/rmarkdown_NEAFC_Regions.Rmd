...
# set the Ecoregion of interest
  s_reg         <- subset(NEAFCReg,NEAFCReg$region == Region_ID)
...
```{r, results='asis'}
  ...
  reg  <- subset(table1,table1$NEAFCregion %in% Region_ID)  
...
```{r, echo=FALSE, message=FALSE, out.width='80%',fig.align = 'center', fig.cap="** **"}
  ...
  reg  <- subset(table1,table1$NEAFCregion == Region_ID)  
  ...
  tab1 <- tab1[,-c(4:5)]
  tab1  %>%
    kable('html',escape = FALSE, align=c('l',rep('c',9)),row.names = FALSE,col.names = c('','All','Mobile gears', ' ','All', 'S1O1',' S1O2', 'S2O1','S2O2','S1O2+S2O1')) %>%
    kable_classic(full_width = F, position = "center",fixed_thead = T,font_size = 10,html_font = "Times") %>%
    ...
    add_header_above(c(" " = 1, "Assessed area" = 1,"Areas fished between 2018-2020" = 1," "=1, "Assessed area" = 1, "In defined VME polygons" = 5 ),bold = TRUE,line_sep = 4,line = TRUE, extra_css = 'padding: 3px;vertical-align:top;') %>%
    add_header_above(c(" " = 1,'Total existing' = 2," "=1, 'New/updated in 2021'=6),bold = TRUE,line_sep = 10, extra_css = 'padding: 3px;vertical-align:top;') %>%
    add_header_above(c(" " = 1,"Number of VME/VME element c-squares" = 9),bold = TRUE, extra_css = 'padding: 3px;vertical-align:top;')
...
```{r echo=FALSE, message=FALSE, out.width='70%',fig.align = 'center', fig.cap="** **"}
  ...
  source(paste(pathdir,"Utilities/Compile_table2_NEAFC.R",sep="/")) # warnings okay
  tab2[is.na(tab2)] <- "" 
  
  tab2[5,2:6] <- NA
  tab2[8,2:6] <- NA
  tab2[10,2:6] <- NA
  display_columns <- c("Scenario 2 Option 1", "Scenario 2 Option 2", "Scenario 1 Option 2 + Scenario 2 Option 1")
  tab2 <- tab2[-c(10,11),!colnames(tab2) %in% display_columns]
  tab2  %>%
...
\*\* For the Barents Sea NEAFC region, depths >50m are also considered.
