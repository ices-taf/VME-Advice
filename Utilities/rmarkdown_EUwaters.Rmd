...
# set the ecoregion of interest
  subreg <- subset(shapeEEZ,shapeEEZ$UNION %in% Region_ID)
  ...
  New_mobile <- st_collection_extract(New_mobile, type = c("POLYGON"),warn = FALSE)
...
# 
  mfs <- leaflet() %>%
   ...
   # boundaries
    ...
    addPolygons(data = Reg_depth, group = nam[2],
                stroke = TRUE, fillOpacity = 0.1, smoothFactor = 0.5, opacity = 0.5, weight = 1, color = "yellow") %>%
    
    # VME c-sqs
    addPolygons(data = VMEgrid_old, group = nam[3],
                stroke = FALSE, fillOpacity = 1, smoothFactor = 0.5,
                color = ~VMEpal(VME_Class_Lab)) %>%
    
    addPolygons(data = VMEgrid_new, group = nam[4],
                stroke = FALSE, fillOpacity = 1, smoothFactor = 0.5,
                color = ~VMEpal(VME_Class_Lab)) %>%
    
    # EU footprint and fishing areas
    addPolygons(data = EUFootp, group = nam[5],
                stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, fillColor =  "white") %>%
    
    addPolygons(data = New_comb, group = nam[6],
                stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, fillColor =  "red") %>%
    
    addPolygons(data = Ref_static, group = nam[7],
                stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, fillColor =  "white") %>%
    
    addPolygons(data = New_static, group = nam[8],
                stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, fillColor =  "red") %>%
    
    addPolygons(data = Ref_mobile, group = nam[9],
                stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, fillColor =  "white") %>%
    
    addPolygons(data = New_mobile, group = nam[10],
                stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, fillColor =  "red") %>%
    
    # elements
    addPolygons(data = Elements, group = nam[11],
                stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, fillColor =  "white") %>%
...
```{r Build option map2, include=FALSE}
 ...
 # find all polygons that intersect with 400-800 m depth 
  scen11_prev <- st_cast(scen11_prev, "POLYGON")  
  scen12_prev <- st_cast(scen12_prev, "POLYGON")  
  scen21_prev <- st_cast(scen21_prev, "POLYGON")  
  scen22_prev <- st_cast(scen22_prev, "POLYGON")  
  
  Sce_O_nam <- c("scen11_prev","scen12_prev","scen21_prev","scen22_prev",
           "scen11","scen12","scen21","scen22","scen23")
  
  for (j in 1:length(Sce_O_nam)){
     overpol <- sf::st_intersects(get(Sce_O_nam[j]),Reg_depth)
     overp   <- as.data.frame(overpol)
     sce_int <- get(Sce_O_nam[j])[overp$row.id,] 
     sce_int <- sce_int[!duplicated(sce_int$geometry), ]
     assign(Sce_O_nam[j],sce_int)
    }
 ...
 mfs2 <- leaflet() %>%
    ...
    addPolygons(data = Reg_depth, group = nam[2],
     ...
     addPolygons(data = scen11_prev, group=nam[3],
    ...
    addPolygons(data = scen12_prev, group=nam[5],
                stroke = TRUE, fillOpacity = 0.1, smoothFactor = 0.5, opacity = 0.5, weight = 2, color = "red") %>%
    addPolygons(data = scen12, group=nam[6],fillColor =  "white", color = "transparent",stroke = FALSE, fillOpacity = 0.5) %>%
    addPolygons(data = scen21_prev, group=nam[7],
                stroke = TRUE, fillOpacity = 0.1, smoothFactor = 0.5, opacity = 0.5, weight = 2, color = "#bcbddc") %>%
    addPolygons(data = scen21, group=nam[8],fillColor =  "white", color = "transparent",stroke = FALSE, fillOpacity = 0.5) %>%
        addPolygons(data = scen22_prev, group=nam[9],
                stroke = TRUE, fillOpacity = 0.1, smoothFactor = 0.5, opacity = 0.5, weight = 2, color = "#c7e9c0") %>%
    addPolygons(data = scen22, group=nam[10],fillColor =  "white", color = "transparent",stroke = FALSE, fillOpacity = 0.5) %>%
    #addPolygons(data = NULL, group=nam[11],
    #            stroke = TRUE, fillOpacity = 0.1, smoothFactor = 0.5, opacity = 0.5, weight = 1, color = "#c7e9c0") %>%
    addPolygons(data = scen23, group=nam[12],fillColor =  "white", color = "transparent",stroke = FALSE, fillOpacity = 0.5) %>%
    
  # existing footprints
    addPolygons(data = EUFootp, group = nam[13],
                stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, fillColor =  "white") %>%
    

    # Layers control
    addLayersControl(
      overlayGroups = nam[c(1:10,12:13)])%>%
  
  # hide 
  hideGroup(nam[c(4:10,12:13)])
```
...
```{r map2 key}
map2layers <- map2layers[-c(1,4,13,14),] 
map2layers[,-2] %>%
