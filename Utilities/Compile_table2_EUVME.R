...
# get fished area in the region
  Mobile_fish <- st_intersection(New_mobile,subreg) ; Mobile_fish <- st_make_valid(Mobile_fish)
  Static_fish <- st_intersection(New_static,subreg) ; Static_fish <- st_make_valid(Static_fish)
...
# calculate overlap with fishable domain
  FD_scen11 <- st_intersection(scen11,Fishdom); FD_scen11 <- st_make_valid(FD_scen11)
  tab2[4,2] <- round(sum(st_area(FD_scen11))/sum(st_area(Fishdom)) * 100,digits = 1)
  
  FD_scen12 <- st_intersection(scen12,Fishdom); FD_scen12 <- st_make_valid(FD_scen12)
  tab2[4,3] <- round(sum(st_area(FD_scen12))/sum(st_area(Fishdom)) * 100,digits = 1)
  
  FD_scen21 <- st_intersection(scen21,Fishdom); FD_scen21 <- st_make_valid(FD_scen21)
  tab2[4,4] <- round(sum(st_area(FD_scen21))/sum(st_area(Fishdom)) * 100,digits = 1)
  
  FD_scen22 <- st_intersection(scen22,Fishdom); FD_scen22 <- st_make_valid(FD_scen22)
  tab2[4,5] <- round(sum(st_area(FD_scen22))/sum(st_area(Fishdom)) * 100,digits = 1)
  
  FD_scen23 <- st_intersection(scen23,Fishdom); FD_scen23 <- st_make_valid(FD_scen23)
...
# Number of VME polygons and their average areal extent (size)
  tab2[6,2] <- paste(nrow(scen11)," (",round(mean(st_area(scen11)/10^6),digits=1)," km<sup>2</sup>)",sep="")
  tab2[6,3] <- paste(nrow(scen12)," (",round(mean(st_area(scen12)/10^6),digits=1)," km<sup>2</sup>)",sep="")
  tab2[6,4] <- paste(nrow(scen21)," (",round(mean(st_area(scen21)/10^6),digits=1)," km<sup>2</sup>)",sep="")
  tab2[6,5] <- paste(nrow(scen22)," (",round(mean(st_area(scen22)/10^6),digits=1)," km<sup>2</sup>)",sep="")
  tab2[6,6] <- paste(nrow(scen23)," (",round(mean(st_area(scen23)/10^6),digits=1)," km<sup>2</sup>)",sep="")

# Number of large VME polygons in upper 25th percentile of the size distribution nd their average areal extent (size)
  sc11areas <- sort(st_area(scen11)/10^6)
  sc12areas <- sort(st_area(scen12)/10^6)
  sc21areas <- sort(st_area(scen21)/10^6)
  sc22areas <- sort(st_area(scen22)/10^6)
  sc23areas <- sort(st_area(scen23)/10^6)
  
  sc11p25 <- which(as.numeric(cumsum(sort(st_area(scen11)/10^6))/sum(st_area(scen11)/10^6))>0.75)
  sc12p25 <- which(as.numeric(cumsum(sort(st_area(scen12)/10^6))/sum(st_area(scen12)/10^6))>0.75)
  sc21p25 <- which(as.numeric(cumsum(sort(st_area(scen21)/10^6))/sum(st_area(scen21)/10^6))>0.75)
  sc22p25 <- which(as.numeric(cumsum(sort(st_area(scen22)/10^6))/sum(st_area(scen22)/10^6))>0.75)
  sc23p25 <- which(as.numeric(cumsum(sort(st_area(scen23)/10^6))/sum(st_area(scen23)/10^6))>0.75)
...
# overlap between static fishing area and vme polygons
  SF_scen11 <- st_intersection(scen11,Static_fish); SF_scen11 <- st_make_valid(SF_scen11)
  tab2[11,2] <- round(sum(st_area(SF_scen11))/sum(st_area(Static_fish)) * 100,digits = 1)
  
  SF_scen12 <- st_intersection(scen12,Static_fish); SF_scen12 <- st_make_valid(SF_scen12)
  tab2[11,3] <- round(sum(st_area(SF_scen12))/sum(st_area(Static_fish)) * 100,digits = 1)
  
  SF_scen21 <- st_intersection(scen21,Static_fish); SF_scen21 <- st_make_valid(SF_scen21)
  tab2[11,4] <- round(sum(st_area(SF_scen21))/sum(st_area(Static_fish)) * 100,digits = 1)
  
  SF_scen22 <- st_intersection(scen22,Static_fish); SF_scen22 <- st_make_valid(SF_scen22)
  tab2[11,5] <- round(sum(st_area(SF_scen22))/sum(st_area(Static_fish)) * 100,digits = 1)
  
  SF_scen23 <- st_intersection(scen23,Static_fish); SF_scen23 <- st_make_valid(SF_scen23)
...
# overlap between mobile fishing area and vme polygons
  SF_scen11 <- st_intersection(scen11,Mobile_fish); SF_scen11 <- st_make_valid(SF_scen11)
  tab2[13,2] <- round(sum(st_area(SF_scen11))/sum(st_area(Mobile_fish)) * 100,digits = 1)
  
  SF_scen12 <- st_intersection(scen12,Mobile_fish); SF_scen12 <- st_make_valid(SF_scen12)
  tab2[13,3] <- round(sum(st_area(SF_scen12))/sum(st_area(Mobile_fish)) * 100,digits = 1)
  
  SF_scen21 <- st_intersection(scen21,Mobile_fish); SF_scen21 <- st_make_valid(SF_scen21)
  tab2[13,4] <- round(sum(st_area(SF_scen21))/sum(st_area(Mobile_fish)) * 100,digits = 1)
  
  SF_scen22 <- st_intersection(scen22,Mobile_fish); SF_scen22 <- st_make_valid(SF_scen22)
  tab2[13,5] <- round(sum(st_area(SF_scen22))/sum(st_area(Mobile_fish)) * 100,digits = 1)
  
  SF_scen23 <- st_intersection(scen23,Mobile_fish); SF_scen23 <- st_make_valid(SF_scen23)
