...
```{r, echo=FALSE, message=FALSE, out.width='80%',fig.align = 'center', fig.cap="** **"}
  ...
  names(tab1) <-c('C1','C2','C3','C4','C5','C6','C7')
  tab1 <- tab1[-1,]
  tab1$C8 <- " "

  tab1 <- tab1 %>% 
            relocate(c('C5','C6','C7','C8'),.after = 'C2')  %>% 
            separate(C4,c('S1O1',' S1O2', 'S2O1',' S2O2', 'S1O2'), sep=',')  %>% 
            select(-S1O2)  %>% 
            replace(is.na(.), '')
  
  # Temporary fix, this table needs checking!
  tab1[14:17,8:11] <-0

  tab1  %>%
    kable('html',escape = FALSE, align=c('l',rep('c',9)),row.names = FALSE,col.names = c('','All','Mobile gears','Static gears',' M + S gears',' ','All', 'S1O1',' S1O2', 'S2O1',' S2O2')) %>%
    kable_classic(full_width = F, position = "center",fixed_thead = T,font_size = 10,html_font = "Times") %>%
    ...
    add_header_above(c(" " = 1, "Assessed area" = 1,"Areas fished between 2018-2020" = 3," "=1, "Assessed area" = 1, "In defined VME polygons" = 4 ),bold = TRUE,line_sep = 4,line = TRUE, extra_css = 'padding: 3px;vertical-align:top;') %>%
    add_header_above(c(" " = 1,'Total existing' = 4," "=1, 'New/updated in 2021'=5),bold = TRUE,line_sep = 10, extra_css = 'padding: 3px;vertical-align:top;') %>%
    add_header_above(c(" " = 1,"Number of VME/VME element c-squares" = 10),bold = TRUE, extra_css = 'padding: 3px;vertical-align:top;')
