...
# set the Ecoregion of interest
  s_reg         <- subset(ICESEcReg,ICESEcReg$Ecoregion == Region_ID)
...
if (length(!is.na(reg$New_VMEs))==0){
  ...
  glue("
  New VME habitat and indicator records were submitted and quality checked.",
  "This resulted in ",
...
```{r, echo=FALSE, message=FALSE, out.width='80%',fig.align = 'center', fig.cap="** **"}
  ...
  reg  <- subset(table1,table1$EEZ %in% Region_ID)  
  ...
  tab1[is.na(tab1)] <- ""
  tab1 %>% 
    kable('html',escape = FALSE, align=c('l',rep('c',11)),row.names = FALSE,col.names = c('','All','Mobile gears','Static gears',' M + S gears',' ','All', 'Mobile gears','Static gears',' M + S gears',' ', 'All')) %>%
    kable_classic(full_width = F, position = "center",fixed_thead = T,font_size = 10,html_font = "Times") %>%
    ...
    column_spec(3:10, width = "2.5cm") %>%
    column_spec(c(2,7,12), width = "3cm") %>%
    column_spec(c(6,11), width = ".6cm") %>%
    add_header_above(c(" " = 1, "Assessed area" = 1,"Areas fished between 2018-2020" = 3," "=1, "Assessed area" = 1, "Areas fished between 2018-2020" = 3, " "=1, "Assessed area" = 1),bold = TRUE,line_sep = 4,line = TRUE, extra_css = 'padding: 3px;vertical-align:top;') %>%
    add_header_above(c(" " = 1,'Existing' = 4," "=1, 'New/updated in 2021'=4, " "=1, 'Total'=1),bold = TRUE,line_sep = 10, extra_css = 'padding: 3px;vertical-align:top;') %>%
...
```{r, echo=FALSE, message=FALSE, out.width='80%',fig.align = 'center', fig.cap="** **"}
  ...
  tab1b  %>%
    kable('html',escape = FALSE, align=c('l',rep('c',6)),row.names = FALSE,col.names = c('','All', 'S1O1',' S1O2', 'S2O1','S2O2','S1O2+S2O1')) %>%
    kable_classic(full_width = F, position = "center",fixed_thead = T,font_size = 10,html_font = "Times") %>%
    row_spec(0, bold = T, extra_css = 'padding: 3.5px;vertical-align:top;')  %>%
    row_spec(c(1,5,9,13),bold = T, background = icespal[['lg']], extra_css = 'padding: 3px;vertical-align:top;') %>%
    row_spec(c(2:4,6:8,10:12,14:nrow(tab1)), extra_css = 'padding: 1.5px;vertical-align:top;') %>%
    ...
    add_header_above(c(" " = 1, "Assessed area" = 1, "In defined VME polygons" = 5),bold = TRUE,line_sep = 4,line = TRUE, extra_css = 'padding: 3px;vertical-align:top;') %>%
    add_header_above(c(" " = 1,'New/updated in 2021'=6),bold = TRUE,line_sep = 10, extra_css = 'padding: 3px;vertical-align:top;') %>%
    add_header_above(c(" " = 1,"Number of VME/VME element c-squares" = 6),bold = TRUE, extra_css = 'padding: 3px;vertical-align:top;')

  tab1_alt <- tab1 %>% 
    select(c("V1","V2", "V3", "V4", "V5", "V6", "V7")) %>% 
             bind_cols(tab1_new_vme)
...
mfs2 <- leaflet() %>%
    ...
    # existing footprints
    addPolygons(data = s_NEAFCFootp, group = nam[17],
                stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, fillColor =  "white") %>%
    
    addPolygons(data = s_NEAFCReg, group = nam[18],
                stroke = TRUE, fillOpacity = 0, smoothFactor = 0.5, opacity = 0.5, weight = 1, color = "red") %>%
    
    addPolygons(data = s_EUFootp, group = nam[19],
                stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, fillColor =  "white") %>%
    
  # VME csquares
  
    addPolygons(data = s_VMEgrid_old, group = nam[5],
                stroke = FALSE, fillOpacity = 1, smoothFactor = 0.5,
                color = ~VMEpal(VME_Class_Lab)) %>%
    
    addPolygons(data = s_VMEgrid_new, group = nam[6],
                stroke = FALSE, fillOpacity = 1, smoothFactor = 0.5,
                color = ~VMEpal(VME_Class_Lab)) %>%
