...
# select all new c-squares
  ...
  # nrow(VMEgrid_tot) # = 3985
  # nrow(VMEgrid_old) # = 3297
  # nrow(VMEgrid_new) # = 699; 699 + 3297 != 3985
  
### Neil edit ###
  
  new_and_changed_cells <- VMEgrid_new %>% dplyr::filter(!VMEgrid_new$uni %in% VMEgrid_old$uni)
  new_vme_cells <- VMEgrid_new %>% dplyr::filter(!VMEgrid_new$csquares %in% VMEgrid_old$csquares)
  changed_cells <- new_and_changed_cells %>% dplyr::filter(!new_and_changed_cells$uni %in% new_vme_cells$uni)
...
# load depths
  ...
  depth$cat <- ifelse(depth$de05_2 ==1 & depth$de2_4 ==0,"de05_2",depth$cat)
  ...
  bargrid <- cbind(bargrid,as.data.frame(new_vme_cells)[match(bargrid@data$csquares,new_vme_cells$csquares),c("VME_Class")])
  ...
  neafc_grid_over <- st_intersects(bargrid_sf, NEAFCReg)
  neafc_idx          <- as.data.frame(neafc_grid_over)
  
  #bargrid@data$NEAFC_region[bargrid@data$csquares %in% unique(bargrid_sf$csquares[idx$row.id])] <- 1
  

  # now add which c-squares are 100% within the old VME polygon
  tt <- st_intersection(new_vme_cells,st_make_valid(scen11_prev))
  ...
  tt <- st_intersection(new_vme_cells,st_make_valid(scen12_prev))
  tt$area <- st_area(st_make_valid(tt))/10^6
  tt$frac <- as.numeric(tt$area/tt$area_sqkm)
  tt <- subset(tt,tt$frac> 0.9)
  bargrid@data$scen12[bargrid@data$csquares %in% unique(tt$csquares)] <- 1
  
  tt <- st_intersection(new_vme_cells,st_make_valid(scen21_prev))
  tt$area <- st_area(st_make_valid(tt))/10^6
  tt$frac <- as.numeric(tt$area/tt$area_sqkm)
  tt <- subset(tt,tt$frac> 0.9)
  bargrid@data$scen21[bargrid@data$csquares %in% unique(tt$csquares)] <- 1
  
  tt <- st_intersection(new_vme_cells,st_make_valid(scen22_prev))
  tt$area <- st_area(st_make_valid(tt))/10^6
  tt$frac <- as.numeric(tt$area/tt$area_sqkm)
  tt <- subset(tt,tt$frac> 0.9)
